...
_.extend(module.exports.prototype, {
	...
	add: function(client) {
		...
		if (!existing) {
			...
			this.broadcast('join', _.pick(client.player.toJSON(), 'id', 'name'));
		}
	...
	deal: function() {
		...
		//this.model.get('deck').reset(_.map([1, 1, 10, 10], function(c) { return new Models.Card({ value: c }) }));
		...
		this.broadcast('deal', {
			dealer: this.model.get('dealer').toJSON().splice(0, 1),
	...
	onSit: function(client, index) {
		...
		this.broadcast('sit', { player: _.pick(client.player.toJSON(), 'id', 'name'), box: index });
	}
});