...
_.extend(module.exports.prototype, {
	...
	onHit: function(client, message) {
		...
		if (score > 21) {
			...
			this.model.set('turn', null);
			return setTimeout(this.nextTurn, settings.timing.bust);
		}
	...
	nextTurn: function() {
		...
		if (turn == null) {
			...
			turn = [next.get('index'), 0];
			debug('turn moves to first box with a hand');
		}
		else if (turn[1] + 1 < this.model.get('boxes').where({ index: turn[0] })[0].get('hands').length) {
			turn[1]++;
			debug('moving to next hand for box');
		} else {
			debug('looking for next box');

			var next = _.find(this.model.get('boxes').rest(turn[0] + 1), function(b) { return !!b.get('hands').length; });

			if (next) {
				debug('moving to next box', next.get('index'));
				turn[0] = next.get('index');
				turn[1] = 0;
			} else {
				debug('no next box');
		...
		var box = this.model.get('boxes').where({ index: turn[0] })[0];
		var hand = box.get('hands').where({ index: turn[1] })[0];
		...
		if (hand.get('cards').score() === 21) {
			...
			return setTimeout(_.bind(function() {
				...
				this.nextTurn();
			}, this), settings.timing.autoStand);
		}
